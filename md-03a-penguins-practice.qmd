...
3.  How many variables are in the data set? 8 (cloumns)

4.  How many observations are in the data set? 344 (rows)
...
5.  What data types are contained in the variables? (Reminder: [https://rbtl-fs24.github.io/website/slides/lec-02-visualisation.html#/types-of-variables](https://ds4owd-001.github.io/website/slides/lec-02-visualisation.html#/types-of-variables){.uri})
    ...
    so I would say there are both numerical (everything that include numbers e.g. bill_length_mm, body_mass_g) and non-numerical (e.g. island, species)
...
When do I use a & and when do I use a coma?
...
penguins_without_chinstraps <- penguins |>
...
Starting with `penguins`, group the data by species, then create a summary table containing the maximum and minimum length of flippers (call the columns flip_max and flip_min). How will you handle NA values? -\> na.rm = TRUE
...
Starting with `penguins`, group the data by species and year, then create a summary table containing the mean bill depth (call this bill_depth_mean), the mean bill length (call this bill_length_mean), and the count for each group. How will you handle NA values? that was hard, needed to peak
