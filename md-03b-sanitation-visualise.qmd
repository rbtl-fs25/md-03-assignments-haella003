...
If you haven't worked with JMP indicators before, the following questions will be challenging to answer. I haven't :/

1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend? they look all the same to me, no changing -\> hard to read anything/get information
2.  What would you want to change? maybe change the scale size? I think we can remove the 99.9 and look only at the difference what changes?
3.  Why did we remove "safely managed sanitation services" from the data set in Task 3? maybe because we want to look where we can improve and not the stations that already work fine?

## Task 7

1.  Run the code in the code chunk below.
2.  What do you observe when you look at the code and plot? maybe that all of the varname_short have a clear tendency except the san_sm which is widely spread. And also that national, rural and urban all have the same tendencies (where the most of the count lies).
